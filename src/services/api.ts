...
import { CATALOG_PRODUCTS, mergeWithCatalog } from "../data/products";
...
export type HealthResponse = { ok: boolean; snapEnabled?: boolean };

export const getHealth = () => request<HealthResponse>("/health");

export const getProducts = async (): Promise<Product[]> => {
  try {
    const remote = await request<Product[]>("/products");
    return mergeWithCatalog(remote);
  } catch {
